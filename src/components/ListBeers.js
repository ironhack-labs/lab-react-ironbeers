...
const ListBeers = (props) => {
    ...
    const renderListOfBeers = () => {
        return beerList.map(beer => {
            return(
                <li>
                    <div key={beer._id} className="container-fluid d-flex mt-3 mb-3 border" style={{width: "60vw"}}>
                        <div className="card-header">
                            <img className="card-img-top" style={{height: "15rem", width: "auto"}} src={beer.image_url} alt={beer.name}/>
                        </div>
                        <div className="card-body">
                            <Link className="text-dark" to={`/beers/${beer._id}`}><h1 className="card-title">{beer.name}</h1></Link>
                            <h2>{beer.tagline}</h2>
